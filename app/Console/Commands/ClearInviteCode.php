...
class ClearInviteCode extends Command
{
    ...
    public function handle()
    {
        ...
        {
            if(($user->plan_id <= 5 || $user->plan_id == 11 || $user->plan_id == 12 || $user->plan_id == 14) && (time() - $user->expired_at) > 604800){
                if (InviteCode::where('user_id', $user->id)->delete()) {
                    $this->info("已删除用户ID为{$user->id}的邀请码");
                }
            }

            if(($user->plan_id == 6 or $user->plan_id == 7 or $user->plan_id == 13) && (time() - $user->updated_at) > 432000){
                if (InviteCode::where('user_id', $user->id)->delete()) {
